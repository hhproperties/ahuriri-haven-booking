      ...
      <section className="bg-[#EFE8DA] px-6 py-24 lg:px-10 lg:py-32">
        <div className="mx-auto max-w-5xl">
          ...
          <div className="grid gap-16 md:grid-cols-2 lg:grid-cols-3">
            {categories.map((cat, i) => (
              <div key={cat.label} className={`reveal-up reveal-stagger-${Math.min(i + 1, 4)}`}>
                <p className="text-[11px] uppercase tracking-[0.24em] font-[Archivo] text-[#6B4630] mb-5">{cat.label}</p>
                <ul className="space-y-3">
                  {cat.items.map((item) => (
                    <li key={item} className="flex items-start gap-3 text-sm font-[Archivo] text-[#17181A]/70">
                      <span className="mt-[6px] h-[5px] w-[5px] shrink-0 rounded-full bg-[#6B4630]/40" />
                      {item}
                    </li>
                  ))}
                </ul>
          ...
          <div className="text-center">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-x-12 gap-y-3 text-sm font-[Archivo] text-[#17181A]/60">
              <span>Check-in from 2:00pm</span>
              <span className="h-px w-6 bg-[#6B4630]/30" />
