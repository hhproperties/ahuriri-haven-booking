...
function LocationPage() {
  ...
  return (
    <div className="min-h-screen bg-[#EFE8DA]">
      ...
      {/* Hero */}
      <section className="relative h-[45vh] min-h-[360px] overflow-hidden bg-[#17181A]">
      ...
      <section className="bg-[#EFE8DA] px-6 py-24 lg:px-10 lg:py-32">
        <div className="mx-auto max-w-5xl space-y-20">
          ...
          {/* Nearby grid */}
          <div>
            <div className="wood-divider mb-12" />
            <p className="reveal-up text-[11px] uppercase tracking-[0.24em] font-[Archivo] text-[#6B4630] mb-8">
              In the neighbourhood
            </p>
            <div className="reveal-up reveal-stagger-1 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {nearby.map((n) => (
                <div key={n.name} className="flex items-baseline justify-between border-b border-[#6B4630]/10 pb-3">
                  <span className="flex items-center gap-2">
                    <span className="font-[Fraunces] text-base text-[#17181A]">{n.name}</span>
                    {n.type && (
                      <span className="text-[10px] uppercase tracking-[0.2em] font-[Archivo] text-[#6B4630]">{n.type}</span>
                    )}
                  </span>
                  <span className="text-[11px] uppercase tracking-[0.2em] font-[Archivo] text-[#6B4630] whitespace-nowrap">{n.distance}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="mt-4 text-sm font-[Archivo] text-[#17181A]/60 max-w-md mx-auto">
